...
{
	if (player.direction == DIR_LEFT && player.x > 0) player.x--;
	if (player.direction == DIR_RIGHT && player.x < 224) player.x++;
}
...
void play(void) {
	...
	player.x =  9*8;
	player.y = 18*8; // The ground! Well, minus 32 pix height
	player.tics = 0;
    player.frame = 0;
